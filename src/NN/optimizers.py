...
class Optimizer(metaclass=ABCMeta):
    ...
    def __init__(self, eta, eps=1e-5):
        """Initialize the optimizer with the specified learning rate :eta: and the
        desire precision :eps:.

        Parameters
        ----------
        eta : float
            Learning rate for SGD optimizer or starting step size for SGM optimizer
        eps : float, optional
...
class SGM(Optimizer):
    ...
    def update_mini_batch(self, nn, mini_batch):
        """Updates weights and biases of the specified Neural Network object :nn: by
        using the current mini-batch samples :mini_batch:. Uses a diminishing square summable
        step size as specified by AdaGrad. Hyperparameters must be configured directly
        on the :nn: object.
