...
class Network(metaclass=ABCMeta):
    ...
    def backpropagation(self, x, y, der, i):
    ...
    def update_mini_batch(self, mini_batch, eta, der, sub=False, batch=False):     
        ...
        nabla_w = [ 0 for w in self.weights ]
        if batch:
            xxx = mini_batch[0].reshape(mini_batch[0].shape[0], -1)
            nabla_b, nabla_w = self.backpropagation_batch(xxx, mini_batch[1], der)
        else:
            i=0
            for x, y in zip(mini_batch[0], mini_batch[1]):
                delta_b, delta_w = self.backpropagation(x, y, der,i)
                i+=1
                nabla_b = [ nb + db.T for nb,db in zip(nabla_b, delta_b)]
                nabla_w = [ nw + dw for nw,dw in zip(nabla_w, delta_w) ]
                self.grad_est += self.g

        # print("single: ",nabla_b[-1])
    ...
    def SGD(self, training_data:tuple, epochs, eta, batch_size=None, test_data:tuple=None, batch=False):
        ...
        for e in range(10):
            ...
            for mini_batch in mini_batches:
                self.update_mini_batch(mini_batch, eta, self.act.derivative, batch=batch)
            ...
            if test_data is not None:
                if batch:
                    x = test_data[0].reshape(test_data[0].shape[0],-1)
                    xxx = training_data[0].reshape(training_data[0].shape[0], -1)
                    y = test_data[1]
                    score, preds_train, preds_test = self.evaluate((x,y), (xxx,training_data[1]), batch=batch)
                else:
